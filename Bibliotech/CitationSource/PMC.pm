...
sub metadata {
  ...
  #
  my $h = HTTP::OAI::Harvester->new(repository => HTTP::OAI::Identify->new(baseURL => PMC_BASE_URL, version=> VERSION));

  #
  # get corresponding record for $art_id from repository
  #
  my ($gr) = $h->GetRecord(identifier => ID_PREFIX.':'.$art_id,  # required
...
sub build_metadata {
    ...
    my $root = $dom->getDocumentElement or $self->errstr('no root');
