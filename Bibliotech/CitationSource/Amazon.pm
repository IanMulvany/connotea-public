...
sub understands
{
    ...
    return 0 unless (
			 ($uri->host eq 'www.amazon.com'
			  || 
			  $uri->host eq 'www.amazon.co.uk'
			  || 
			  $uri->host eq 'www.amazon.de'
			  || 
			  $uri->host eq 'www.amazon.fr'
			  || 
			  $uri->host eq 'www.amazon.co.jp'
                          ||
                          $uri->host eq 'www.amazon.ca'
			  ) 
			 &&
			 ( $uri->path =~ m!/\d{9}(?:\d|X)/!i )
			 );
    return 1;
}
