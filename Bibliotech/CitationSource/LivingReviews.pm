...
sub citations {
     ...
     return new Bibliotech::CitationSource::ResultList(Bibliotech::CitationSource::Result::LivingReviews->new($raw_citation));
}
...
sub authors {
    my $self = shift;
    return new Bibliotech::CitationSource::Result::AuthorList(map { Bibliotech::CitationSource::Result::Author::LivingReviews->new($_) } @{$self->{'citation'}->{'authors'}} );
}
