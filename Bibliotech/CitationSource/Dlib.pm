...
sub understands
{
    ...
    return 0 unless (
			 ($uri->host eq 'www.dlib.org'
			  || 
			  $uri->host eq 'dlib.org') 
			 &&
			 ( $uri->path =~ m!^/dlib/(?:january|february|march|april|may|june|july|august|september|october|november|december)\d+/(.+?)/\d+\1\.html$!i )
			 );
    return 1;
}
...
sub citations
{
     ...
     return new Bibliotech::CitationSource::ResultList(Bibliotech::CitationSource::Result::Dlib->new($raw_citation));
}
...
sub dlib_meta_uri
{
    ...
    return new URI($new_uri);
}
sub raw_parse_dlib_xml
{
    my ($self, $xml) = @_;

    ##evil regex XML parsing coming up
